...
@if (session('deliveryStart'))
<div class="alert alert-success alert-dismissible fade show" role="alert">
     {{ session('deliveryStart') }}
</div>
@endif

<?php $data = session('newOrdersData');?>
...
<table class="container mt-5 table center table-striped table-hover table-responsive">
  ...
  @foreach ($data as $alldata)
    ...
    <tr>
      ...
      <td>
        <form action="ordersStart" method="POST">
        @csrf
    ...
    @if($i == 1)
    <div class=" text-center mt-5">No New Orders</div>
    @endif
